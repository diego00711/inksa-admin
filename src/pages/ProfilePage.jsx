...
import { Lock, Clock, CheckCircle, AlertCircle, Camera, User, Mail, Briefcase, Calendar, Edit, Save, X, Phone } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const [editForm, setEditForm] = useState({ name: '', cargo: '', phone: '' });
  const [editSaving, setEditSaving] = useState(false);

  const startEdit = () => {
    setEditForm({
      name: profile?.name || '',
      cargo: profile?.cargo || profile?.role || '',
      phone: profile?.phone || '',
    });
    setIsEditing(true);
  };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="flex items-start justify-between gap-2">
              ...
              {!isEditing && (
                <button
                  type="button"
                  onClick={startEdit}
                  className="flex items-center gap-1.5 text-sm text-indigo-600 hover:text-indigo-800 px-3 py-1.5 rounded-md hover:bg-indigo-50 transition"
                >
        ...
        {isEditing ? (
          <form onSubmit={saveProfile} className="space-y-4 pt-4 border-t border-gray-100">
            <div>
              ...
              <input
                ...
                className="block w-full rounded-md border border-gray-300 shadow-sm px-3 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                placeholder="Ex: Diego Weber"
                maxLength={120}
              />
            </div>
            <div>
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              <button
                ...
                onClick={cancelEdit}
                disabled={editSaving}
                className="flex items-center gap-1.5 px-4 py-2 bg-gray-100 text-gray-700 text-sm font-medium rounded-md hover:bg-gray-200 transition-colors"
              >
